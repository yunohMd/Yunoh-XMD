...
const getChatbotStatus = async () => {
  try {
    const filePath = path.join(storeDir, chatbotFile);
    const data = await fs.readFile(filePath, 'utf8');
    return JSON.parse(data);
  } catch {
    return {};
...
const setChatbotStatus = async (jid, value) => {
  ...
  await writeJSON(chatbotFile, status);
};

const isChatbotEnabled = async (jid) => {
  const status = await getChatbotStatus();
  return status[jid] === true;
};
...
const getChatSummary = async () => {
  ...
  return summaries.sort((a, b) => new Date(b.lastMessageTimestamp) - new Date(a.lastMessageTimestamp));
};
